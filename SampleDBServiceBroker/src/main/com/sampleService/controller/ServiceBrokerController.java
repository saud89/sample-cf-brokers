...
import main.com.sampleService.DBTableService.ControllerBackendOps;
...
public class ServiceBrokerController {
	...
	private static DBTableService dbService = new DBTableService();
	...
	public Response createServiceInstance(@PathParam("instance_id") String instanceId,
		...
		case ControllerBackendOps.SERVICE_INSTANCE_DOESNT_EXISTS:
			...
			//hashService.createHashMap(instanceId);
			dbService.createTable(instanceId);
			responseStatus = 201;
			response.put("dashboardurl", ControllerBackendOps.uri+"/"+instanceId);
			break;
		}
	...
	public Response createSomething(ServiceBinding sb){
		JSONObject response = new JSONObject();
		response.put("service_id", sb.getService_id());
		response.put("plan_id", sb.getPlan_id());
	...
	public Response deleteService(@PathParam("instance_id") String serviceInstanceId,
		...
		JSONObject response = new JSONObject();
		response.put("", "");
		//hashService.deleteHashMap(serviceInstanceId);;
		dbService.deleteTable(serviceInstanceId);
